...
int main(int argc, char **argv)
{
   ...
   strcpy(indxModeStr, argv[1]);
        ...
        if(strcmp(indxModeStr, "HTM") == 0) indxMode = HTM;
   else if(strcmp(indxModeStr, "HPX") == 0) indxMode = HPX;
   else
   {
   ...
   if(srchMode == CONE)   // CONE search arguments
   {
      ...
      constraints = sptConeSearch(indxMode, level, rac, decc, rad);
   ...
   else   // POLYGON search arguments
   {
      ...
      constraints = sptPolygonSearch(indxMode, level, npoly, ra, dec);
