...
struct sptConstraints sptConeSearch(int indexMode, int level, double ra, double dec, double radius)
{
   int    i, ind, nranges;
   ...
   if(indexMode == HTM)
   {
      ...
      while(1)
      {
         if(list[id].min == list[id].max)
         {
            ...
            if(i == 0)
               sprintf(tmpstr, "(htm%d = %" PRId64 ")", level, list[id].min);
            else
               sprintf(tmpstr, " OR (htm%d = %" PRId64 ")", level, list[id].min);
         }
         else
         {
            ...
            if(i == 0)
               sprintf(tmpstr, "(htm%d BETWEEN %" PRId64 " AND %" PRId64 ")", level, list[id].min, list[id].max);
            else
               sprintf(tmpstr, " OR (htm%d BETWEEN %" PRId64 " AND %" PRId64 ")", level, list[id].min, list[id].max);
         }
   ...
   else  // HPX mode
   {
      ...
      while(1)
      {
         if(list[id].min == list[id].max)
         {
            ...
            {
               if(i == 0)
                  printf("      (hpx%d = %" PRIu64 ")                         // %" PRIu64 "\n",
                        level, list[id].min, htm_idtodec(list[id].min));
               else
                  printf("   OR (hpx%d = %" PRIu64 ")                         // %" PRIu64 "\n",
                        level, list[id].min, htm_idtodec(list[id].min));
            }
            
            if(i == 0)
               sprintf(tmpstr, "(hpx%d = %" PRIu64 ")", level, list[id].min);
            else
               sprintf(tmpstr, " OR (hpx%d = %" PRIu64 ")", level, list[id].min);
         }
         else
         {
            newtotal += list[id].max - list[id].min + 1.;
            ...
            {
               if(i == 0)
                  printf("      (hpx%d BETWEEN %" PRIu64 " AND %" PRIu64 ")  // %" PRIu64 " to  %" PRIu64 "\n",
                        level, list[id].min, list[id].max, htm_idtodec(list[id].min), htm_idtodec(list[id].max));
               else
                  printf("   OR (hpx%d BETWEEN %" PRIu64 " AND %" PRIu64 ")  // %" PRIu64 " to  %" PRIu64 "\n",
                        level, list[id].min, list[id].max, htm_idtodec(list[id].min), htm_idtodec(list[id].max));
            }
            
            if(i == 0)
               sprintf(tmpstr, "(hpx%d BETWEEN %" PRIu64 " AND %" PRIu64 ")", level, list[id].min, list[id].max);
            else
               sprintf(tmpstr, " OR (hpx%d BETWEEN %" PRIu64 " AND %" PRIu64 ")", level, list[id].min, list[id].max);
         }
   ...
   if(sptDebug)
   {
      ...
      printf("WHERE (%.17f*x)+(%.17f*y)+(%.17f*z)>=%.17e\n\n", vec.x, vec.y, vec.z, cosrad);

      printf("# ---------------------------------------------------------\n\n");
      fflush(stdout);
   }

   sprintf(constraints.geomConstraint, "(%.17f*x)+(%.17f*y)+(%.17f*z)>=%.17e", vec.x, vec.y, vec.z, cosrad);
...
struct sptConstraints sptPolygonSearch(int indexMode, int level, int npoly, double *ra, double *dec)
{
   int    i, flip, ind, nranges;
   ...
   if(indexMode == HTM)
   {
      ...
      for(i=0; i<npoly; ++i)
      {
         ...
         if(sptDebug)
         {
            printf("(RA,Dec) = (%.6f,%.6f), ",      ra[i], dec[i]);
   ...
   sprintf(tmpstr, "(%.17f*x)+(%.17f*y)+(%.17f*z)<=0.", normal[0].x, normal[0].y, normal[0].z);

   strcat(constraints.geomConstraint, tmpstr);

   for(i=1; i<npoly; ++i)
   {
      sprintf(tmpstr, " AND (%.17f*x)+(%.17f*y)+(%.17f*z)<=0.", normal[i].x, normal[i].y, normal[i].z);
...
int hpxPolygonSearch(int omax, int npoly, double *ra, double *dec, struct hpxRanges **hpxrng)
{
   ...
   crlimit = (double ***)malloc(omax * sizeof(double **));
